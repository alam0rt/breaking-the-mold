...
void SetRenderSpriteRotationPivot(RenderSpriteObj *spr, s16 pivotX, s16 pivotY) {
    spr->pivotX = pivotX;
    spr->pivotY = pivotY;
}
...
u16 GetRenderSpriteRotation(RenderSpriteObj *spr) {
    return spr->rotationAngle;
}

/* Setter for the u16 at +0x28 (see GetRenderSpriteRotation). */
void SetRenderSpriteRotation(RenderSpriteObj *spr, u16 value) {
    spr->rotationAngle = value;
}

/* Getter: s32 at +0x20 - the 16.16 render scale Y (1.0 = 0x10000). */
s32 GetRenderSpriteScaleY(RenderSpriteObj *spr) {
    return spr->scaleY;
}

/* Getter: s32 at +0x1C - the 16.16 render scale X (1.0 = 0x10000). */
s32 GetRenderSpriteScaleX(RenderSpriteObj *spr) {
    return spr->scaleX;
}

/* Setter for the s32 at +0x20. */
void SetRenderSpriteScaleY(RenderSpriteObj *spr, s32 value) {
    spr->scaleY = value;
}

/* Setter for the s32 at +0x1C. */
void SetRenderSpriteScaleX(RenderSpriteObj *spr, s32 value) {
    spr->scaleX = value;
}

/* Seed both 16.16 scale slots at +0x1C/+0x20 to the same value - the
 * uniform-scale reset (scaleX = scaleY = X). */
void SetRenderSpriteScaleUniform(RenderSpriteObj *spr, s32 value) {
    spr->scaleX = value;
    spr->scaleY = value;
}
...
void SetRenderSpriteColorModulate(RenderSpriteObj *spr, u8 value) {
    spr->colorModulate = value;
}

/* Getter: sprite tint RGB at +0x34..+0x36 (the per-vertex color the GPU
 * modulates the textured quad with). */
void GetRenderSpriteColor(RenderSpriteObj *spr, u8 *outR, u8 *outG, u8 *outB) {
    *outR = spr->r;
    *outG = spr->g;
    *outB = spr->b;
}
...
void SetRenderSpriteColor(RenderSpriteObj *spr, u8 r, u8 g, u8 b) {
    spr->r = r;
    spr->g = g;
    spr->b = b;
}
...
void SetRenderSpriteClut(RenderSpriteObj *spr, u16 value) {
    spr->clut = value;
}
...
void SetRenderSpriteTPageAndTexBytes(RenderSpriteObj *spr, u16 tpage, u8 val30, u8 val31) {
    spr->dirty = 1;
    spr->texU = val30;
    spr->texV = val31;
...
void SetSpriteTPageFromVRAMCoords(RenderSpriteObj *spr, u8 abr) {
    s32 x = spr->vramX;
    s32 y = spr->vramY;
