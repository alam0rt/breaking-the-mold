 ...
 * with GCC 2.7.2, so the decompiled C is provided for documentation only.
 * 
 ...
 *     CdControlF(0x02, &pos, NULL);  // CdlSetloc
 ...
 *     return CdControlF(0x1B, &(&g_Music1File)[g_MusicFileIndex], NULL) == 1;
 * }
 */
 ...
 *     u16 sectorOffset = D_8009B3EC[(fileIndex * 8) + channel];
 *     s32 targetOffset = (channel == 0) ? sectorOffset - 3 : sectorOffset - 2;
 *     
 *     D_800A5A04 = 0xC0;  // CD mode: speed + XA-ADPCM
 *     g_MusicTargetSector = g_Music1Sector[fileIndex] + (targetOffset * 8);
 *     CdControlF(0x0E, &D_800A5A04, NULL);  // CdlSetmode
 *     
 *     for (int retries = 8; retries > 0; retries--) {
 *         g_MusicCurrentSector = g_Music1Sector[g_MusicFileIndex];
 *         if (CdControlF(0x1B, &g_Music1File[g_MusicFileIndex], NULL) == 1) {
 *             D_800A5A04 = 0xC8;  // realtime mode
 *             CdControlF(0x0E, &D_800A5A04, NULL);
 *             D_800A5A0C = 1;
 *             D_800A5A0D = channel;
 *             CdSetFilter(0x0D, &D_800A5A0C, &D_800A59FC);
 *             g_MusicState = 1;
 *             return 1;
 *         }
 *     }
 ...
 *     if (g_AllMusicFilesFound == 0) return 0;
 *     CdControlF(0x08, NULL, NULL);  // CdlStop
 *     CdCallback();
 *     g_MusicState = 0;
 *     return 1;
 * }
 ...
 *     if (g_AllMusicFilesFound == 0) return 0;
 *     if (g_MusicPaused == 0) {
 *         CdControlF(0x09, NULL, NULL);  // CdlPause
 *         g_MusicPaused = 1;
 *     }
 *     return 1;
 * }
 ...
 *     CdControlF(0x1B, NULL, NULL);  // CdlReadS
 *     g_MusicPaused = 0;
 *     return 1;
 * }
 */
 ...
 *     if (g_MusicPaused) { CdControlF(0x09, NULL, NULL); return 1; }
 *     if (!g_MusicState || !g_AllMusicFilesFound) return 1;
 *     
 *     s32 status = CdStatus(1, &loc);
 *     switch (status) {
 *         case 2:  // Playing
 *             if (g_MusicTargetSector < g_MusicCurrentSector) {
 *                 // Loop back to start
 *                 g_MusicCurrentSector = g_Music1Sector[g_MusicFileIndex];
 *                 CdControlF(0x1B, &g_Music1File[g_MusicFileIndex], NULL);
 *                 g_MusicRetryCount = 0;
 *             } else {
 *                 // Update position
 *                 if (CdGetlocP(&loc) == 0x11) {
 *                     s32 sector = CdPosToInt(&loc);
 *                     if (sector > 0) g_MusicCurrentSector = sector;
 *                 }
 *                 CdControlB(0x11, NULL);
 *             }
 *             break;
 *         case 5:  // Error
 ...
 *             if (CdControlF(0x15, pos, NULL)) goto seek_ok;  // CdlSeekL
 *         }
 *         return 0;
 *     seek_ok:
 ...
 *     CdSetFilter(0x09, NULL, NULL);  // CdlPause
 * }
 */
