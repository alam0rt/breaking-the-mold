...
def export_symbols_from_ghidra(name_filter: Optional[str] = None) -> str:
    """
    Export function symbols from Ghidra to symbol_addrs.txt format.
    
    Args:
        name_filter: Optional substring filter for function names
    ...
    # Build query URL
    if name_filter:
        result = ghidra_request(f"/functions?name_contains={name_filter}&limit=1000")
    else:
        result = ghidra_request("/functions?limit=2000")
    ...
    lines = []
    lines.append(f"// Symbol addresses exported from Ghidra")
    ...
    for func in sorted_funcs:
        ...
        # Skip unnamed/auto-generated functions
        if not name or name.startswith("FUN_") or name.startswith("LAB_"):
            continue
            
        # Convert address to 0x80XXXXXX format
        addr = int(addr_str, 16)
        if addr < 0x80000000:
            addr |= 0x80000000
        
        # Try to get function size from disassembly
        size = 0
        try:
...
def main():
    ...
    if args.export_symbols:
        code = export_symbols_from_ghidra(args.symbol_filter)
        print(code)
        return
