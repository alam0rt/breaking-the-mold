...
/*
 * klash_match_raw: like klash_match but uses RAW namespace (id = calcHash).
 *
 * Use for sprite/audio assets whose ids are unwrapped calcHash values
 * (~99% of BLB assets). Wrap namespace is only used for the 6 menu anchors.
 *
 *   gcc -O3 -o klash_match_raw tools/klash_match_raw.c
 *   echo "FINN_DOWN" | ./klash_match_raw <ids-file>
 *
 * Output: "0x<id>\t<candidate>\tfloor=F\tlen=L"
 *   floor = popcount(id), len = effective alnum char count.
 *   --max-overshoot N: only print hits where (L - floor) in [0, N].
 */

static int popcount32(uint32_t v){ int c=0; while(v){ v&=v-1; c++; } return c; }
static int floor_of(uint32_t id){ return popcount32(id); }
...
int main(int argc, char **argv){
    const char *ids_path=NULL; int max_overshoot=-1; int wordlike=0;
    ...
    if(!ids_path){ fprintf(stderr,"usage: %s [--max-overshoot N] [--wordlike] [--max-cons C] <ids-file>  (raw namespace, candidates on stdin)\n", argv[0]); return 1; }
    ...
    fclose(f);
    fprintf(stderr,"loaded %zu raw target ids", added);
    if(max_overshoot>=0) fprintf(stderr," (overshoot<=%d)", max_overshoot);
    fputc('\n', stderr);

    uint64_t tried=0, hit=0;
    while(fgets(line,sizeof line,stdin)){
        line[strcspn(line,"\r\n")]='\0';
        uint32_t id=calcHash(line);
        if(set_has(id)){
