...
|---------|-------|-------------|----------|
| **Primary** (shared) | Per-level (shared) | 600, 601, 602 | Level geometry, audio samples |
| **Secondary** (tileset) | Per-stage | 100-401 | Tiles, tile metadata, palettes |
| **Tertiary** (map) | Per-stage | 100-700 | Stage data, entities, sprites, audio |
...
> arrays, so they pair 1:1 per stage: secondary[N] is stage N's tile/palette
> **graphics bank** (the artwork), tertiary[N] is stage N's **map** (tilemap + collision +
> entity placement).
>
> The ImHex template (`docs/blb.hexpat`) now emits **descriptive segment keys**, and
> `tools/extract_blb` lays the output out by world → stage → {tileset, map}:
>
> | JSON segment key | Extracted folder | Meaning |
> |------------------|------------------|---------|
> | `shared` | `<WORLD>/shared/` | world-shared geometry + audio |
> | `stageN_tileset` | `<WORLD>/stageN/tileset/` | per-stage tile/palette graphics |
> | `stageN_map` | `<WORLD>/stageN/map/` | per-stage tilemap, collision, entities |
>
> The extractor also still accepts the older positional keys
> (`primary` / `secondary[N]` / `stageN`) so previously-exported JSON keeps working.
